...
const Card = () => {
  ...
  useEffect(() => {
    ...
    Axios.get(url)
      .then(() => {
        console.log("Axios request for URL 1 successful");
      })
      .catch((error) => {
        console.error("Axios request for URL 1 failed", error);
      });
    return () => {};
  }, []);
  ...
  const getFieldError = (field, name) => {
    return errors[field] && `Please fill in ${name}`;
  };
              ...
              <article className="content">
                ...
                {isLoader ? (
                  <div className="box box-height pl-5 pr-5">
                    ...
                    <p className="data-check">Checking your information...</p>
                  </div>
                ) : (
                  ""
                )}
